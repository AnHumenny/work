...
class AccidentOpenView(ListView):
    model = Accident
    queryset = Accident.objects.order_by("-id").filter(status="open") 
    paginate_by = 20

class AccidentCloseView(ListView):
    model = Accident
    queryset = Accident.objects.order_by("-id").filter(status="close")  
    paginate_by = 20

class AccidentCheckView(ListView):
    model = Accident
    queryset = Accident.objects.order_by("-id").filter(status="check")  # Это ключевой запрос
    paginate_by = 20
...
def listing(request):
    ...
    paginator = Paginator(inc, 25)  # Show 25  per page.
