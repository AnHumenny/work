...
#current_exchange
@router.message(Command("current_exchange"))
async def current_exchange(message: types.Message):
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="USD",
        callback_data="USD")
...
async def send_current_exchange(callback: types.CallbackQuery):
    time_str = time.strftime("%Y-%m-%d %H:%M:%S")
    l = [0, Registred.name, time_str, f"посмотрел курс USD"]
    await Repo.insert_into_date(l)
    ...
    await callback.message.answer(f"Текущий курс \n {res}")
...
async def send_random_value(callback: types.CallbackQuery):
    answer = await Repo.select_manual(2)
    time_str = time.strftime("%Y-%m-%d %H:%M:%S")
    l = [0, Registred.name, time_str, f"посмотрел курс EURO"]
    await Repo.insert_into_date(l)
    await callback.message.answer(f"{answer.tip} \n {answer.comment}")
...
async def send_random_value(callback: types.CallbackQuery):
    answer = await Repo.select_manual(3)
    time_str = time.strftime("%Y-%m-%d %H:%M:%S")
    l = [0, Registred.name, time_str, f"посмотрел курс RUR"]
    await Repo.insert_into_date(l)
    await callback.message.answer(f"{answer.tip} \n {answer.comment}")
...
async def send_random_value(callback: types.CallbackQuery):
    answer = await Repo.select_manual(4)
    time_str = time.strftime("%Y-%m-%d %H:%M:%S")
    l = [0, Registred.name, time_str, f"посмотрел курс CNY"]
    await Repo.insert_into_date(l)
    await callback.message.answer(f"{answer.tip} \n {answer.comment}")
